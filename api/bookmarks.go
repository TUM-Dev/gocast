...
func (r bookmarkRoutes) Add(c *gin.Context) {
	...
	err = r.BookmarkDao.Add(c, &bookmark)
	if err != nil {
		_ = c.Error(tools.RequestError{
...
func (r bookmarkRoutes) GetByStreamID(c *gin.Context) {
	...
	user = c.MustGet("TUMLiveContext").(tools.TUMLiveContext).User
	bookmarks, err = r.BookmarkDao.GetByStreamID(c, query.StreamID, user.ID)
	if err != nil {
...
func (r bookmarkRoutes) Update(c *gin.Context) {
	...
	user = c.MustGet("TUMLiveContext").(tools.TUMLiveContext).User
	bookmark, err = r.BookmarkDao.GetByID(c, uint(id))
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			_ = c.Error(tools.RequestError{
				Status:        http.StatusNotFound,
	...
	err = r.BookmarkDao.Update(c, &bookmark)
	if err != nil {
		_ = c.Error(tools.RequestError{
			Status:        http.StatusInternalServerError,
...
func (r bookmarkRoutes) Delete(c *gin.Context) {
	...
	err = r.BookmarkDao.Delete(c, uint(id))
	if err != nil {
		_ = c.Error(tools.RequestError{
			Status:        http.StatusInternalServerError,
