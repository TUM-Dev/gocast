export PROJ_PATH=TUM-Live-Backend
...
##
####################################################################################################################
all: app test ## build example and run tests

binaries: app ## build binaries in bin dir
...
app: build_info ## build example binary in bin dir
	@echo "build server"
	make BIN_NAME=app APP_PATH=$(PROJ_PATH)/app/server build_app
	@echo ''
	@echo ''
...
regen: ## regenerate generated code
	gen \
     --sqltype=postgres \
     --connstr='host=localhost port=5432 user=joscha database=rbglive password=somepassword' \
     --database=rbglive \
