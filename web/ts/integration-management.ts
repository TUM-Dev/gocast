...
export class IntegrationAdmin {
    ...
    async create() {
        this.beginRequest();
        try {
            const response = await postData("/api/v2/admin/integrations", {
                name: this.name,
    ...
    async rotate(row: IntegrationRow) {
        this.beginRequest();
        try {
            const response = await postData(`/api/v2/admin/integrations/${row.id}/key`);
            if (!response.ok) {
                const error: { message?: string } | null = await response.json().catch(() => null);
                this.error = error?.message || `The request failed (${response.status}). Try again.`;
                return;
    ...
    async revoke(row: IntegrationRow) {
        this.beginRequest();
        try {
            const response = await fetch(`/api/v2/admin/integrations/${row.id}/key`, { method: "DELETE" });
            if (!response.ok) {
                const error: { message?: string } | null = await response.json().catch(() => null);
                this.error = error?.message || `The request failed (${response.status}). Try again.`;
                return;
