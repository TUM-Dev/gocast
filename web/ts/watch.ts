...
export const repeatHeatMap = {
    heatMapPath: heatMapExamplePath,
    init() {
        ...
        //this.heatMapPath = this.genHeatMapPath();
    },

    show() {
        const event = new CustomEvent("updateheatmap", {
            detail: { visible: true, seekbar: this.getSeekbarInfo(), heatMapPath: this.heatMapPath },
        });
        window.dispatchEvent(event);
    },
    ...
    genHeatMapPath() {
        let res = "M 0.0,100.0";
        for (let x = 0; x <= 100; x++) {
            const p = ` C ${x.toFixed(1)},89.5 2.0,55.4 5.0,47.3`;
            res += p;
        }
        return res;
    },
