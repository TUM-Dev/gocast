...
func stream(streamCtx *StreamContext) {
	...
	for time.Now().Before(streamUntil) && !streamCtx.stopped {
		...
		if strings.Contains(streamCtx.sourceUrl, "rtsp") {
			...
			cmdStr := fmt.Sprintf(
				`ffmpeg -hide_banner -nostats -rtsp_transport tcp -t %s -i %s -map 0 -c copy -f mpegts - -c:v libx264 -preset veryfast -tune zerolatency -maxrate 2500k -bufsize 3000k -g 60 -r 30 -x264-params keyint=60:scenecut=0 -c:a aac -ar 44100 -b:a 128k -f flv %s >> %s`,
				timeout,
				streamCtx.sourceUrl,
				fmt.Sprintf("%s/%s", streamCtx.ingestServer, streamCtx.streamName),
				streamCtx.getRecordingFileName(),
			)
			cmd = exec.Command("sh", "-c", cmdStr)
		} else {
			timeout := fmt.Sprintf("%.0f", time.Until(streamUntil).Seconds())
			cmdStr := fmt.Sprintf(
				`ffmpeg -hide_banner -nostats -rw_timeout 5000000 -t %s -i %s -map 0 -c copy -f mpegts - -c:v libx264 -preset veryfast -tune zerolatency -maxrate 2500k -bufsize 3000k -g 60 -r 30 -x264-params keyint=60:scenecut=0 -c:a aac -ar 44100 -b:a 128k -f flv %s >> %s`,
				timeout,
				streamCtx.sourceUrl,
				fmt.Sprintf("%s/%s", streamCtx.ingestServer, streamCtx.streamName),
				streamCtx.getRecordingFileName(),
