...
services:
  ...
  rtmp-proxy:
    image: docker.io/carlobortolan/rtmp-proxy:latest
    volumes:
      - /srv/cephfs/livestream/TUM-Live-Beta/deployment/rtmp-proxy/rtmp-nginx.conf:/etc/nginx/nginx.conf:ro
      - /srv/cephfs/livestream/TUM-Live-Beta/deployment/rtmp-proxy/rtmp-publish.sh:/etc/nginx/scripts/publish.sh
    deploy:
      mode: global
      placement:
        constraints:
          - "node.labels.rtmpproxy==true"
    labels:
        - "traefik.enable=true"

        - "traefik.tcp.routers.rtmpproxy.entrypoints=rtmp"
        - "traefik.tcp.routers.rtmpproxy.rule=HostSNI(`proxy.your-urlc>`)"
        - "traefik.tcp.routers.rtmpproxy.service=rtmp-proxy"

        - "traefik.tcp.services.rtmpproxy.loadbalancer.server.port=1935"
...
wikiURL: https://docs.live.rbg.tum.de
rtmpProxyURL: rtmp://proxy.example.com/live
```
